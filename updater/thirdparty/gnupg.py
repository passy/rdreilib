...
class GPG(object):
    ...
    def verify_detached(self, filename, sign):
        ...
        with NamedTemporaryFile(delete=False, suffix='.asc') as signature:
            signature.write("-----BEGIN PGP SIGNATURE-----\n")
            signature.write("Version: GnuPG v1.4.9 (GNU/Linux)\n\n")
            signature.write(sign)
            signature.write("\n-----END PGP SIGNATURE-----\n")
            signature_fname = signature.name

        result = Verify()
        args = ['--verify', signature.name, filename]
        try:
